...
function crackLiftQuest(){
	...
	timer=setInterval(()=>{
		...
		lift=false	
		gameOver()
	}, levelObj.timeScreamInCrackLift)
...
img.onclick=function (e){
	...
	if(closing){
		...
		t=setInterval(function (){
			durabilityTool.phone++;
			if(durabilityTool.phone==levelObj.durabilityTools.phone){
				woringTool.phone=false
				//return
			}
			//d()
...
document.getElementById('yes').onclick=function(){
	levelObj=getByName(currentLevel);
	first=levelObj.first;
	
	
	countClickedMonsters=levelObj.valueMonstersInCrackLift
	selectLevel.style.display='none'
	setTimeout(()=>{
		closeBlendMonstersId.disabled=false
	}, 15000)
	...
	sc.add('crackLift', './lift.glb', (obj)=>{
		...
		setTimeout(()=>{
			...
			let selectAudio=setInterval(function(){
				if(parseFloat(crackLift.style.opacity)==0.3)
					clearInterval(selectAudio)
	...
	sc.add('liftEnd', './lift.glb', ()=>{
		...
		setTimeout(()=>{
			...
			if(!isMute){
				...
				new Audio(audio.crackLift2).play()
				new Audio(audio.crackLift3).play()
				napr=new Audio('./audionaprag.mp3')
				napr.play()
				napr.volume=1
				rec=new Audio('./rec.mp3')
				rec.play()
				rec.volume=0.1
			}
			rec.onended=()=>{
			...
			crackLiftlightTest.color.r=255
			crackLiftlightTest.color.g=79
			crackLiftlightTest.color.b=0
			setInterval(function(){
			...
			console.log(crackLiftlightTest)
		}, 15000)
	}, null)
	...
	/*for(let i=0;i<2;i++){
		...
		x=x||3
		y=y||4
		let monster=new Monster(x,y, 'normal');//normal or blend
		monsters.push(monster)
	}*/
	let r=rand(1, 6);
	let x;
	let y;
	for(let i=0; i<levelObj.valueMonsters; i++){
		if(r%2)
			x=r
		else
			y=r
	...
	for(let i=0; i<levelObj.valueBlendMonsters; i++){
		if(r%2)
			x=r
		else
			y=r
		x=x||3
		y=y||4
		let monster=new Monster(x,y, 'blend');
		monsters.push(monster)
	}
	...
	render.domElement.onmousedown=function(){
		...
		monsters.forEach((monst, k)=>{
			if(monst.for3D&&
			monst.for3D.pos==currentPosition&&
			monst.type=='normal'){
				time=setTimeout(()=>{
		...
		t=setInterval(function d(){
			...
			if(durabilityTool.flashlight==levelObj.durabilityTools.flashlight){
	...
	document.body.onkeydown=function(e){
		...
		if(e.key=='h'||e.key=='р'){
			if(!isMute){
				new Audio(audio.shok).play()
			}
			if(usedShoks==countShoksOnMonsters)
				return
			usedShoks++
			let objectShokedMonster;
			let lightTestShok = new THREE.SpotLight(0xFFFFFF);
			lightTestShok.position.y=camera.position.y-15;
			lightTestShok.position.z=camera.position.z-30;
			light.position.x=camera.position.x-50;
			
			scene.add(lightTestShok)
			let countLight=0;
			let t=setInterval(function(){
				countLight++
				if(countLight>=100){
					scene.remove(lightTestShok)
					scene.remove(objectShokedMonster)
					clearInterval(t)
				}
					
				let r=Math.random()*0.4
				setTimeout(function(){
					lightTestShok.intensity=r
				}, 10)
			}, 30)
			monsters.forEach((monst, k)=>{
				if(monst.for3D&&
				monst.for3D.pos==currentPosition){
					objectShokedMonster=monst.for3D.object
					monst.for3D.stop=true
					monst.isAudio=false
					monst.is3D=false
				}	
		...
		if (intersects.length > 0) {
			...
			if(intersects[0].object.name=='knopka'){
				...
				if(!first){
					...
					if(random<levelObj.changeCrackLift)
						sc.scene('crackLift')
					else
						sc.scene('lift')
				}else
	...
	table.addEventListener('click', function(e){
		...
		function stopedMonsters(){
			positionAudio.forEach((v, k)=>{
				monsters.forEach((monst, k)=>{
					...
					if(monst.x==Number(positionAudioXY[0]) && monst.y==Number(positionAudioXY[1]) && monst.type=='normal'){
						var d = Math.random();
						if (d > 0.6&&isAudioMonsters.length<2){
							monst.isAudio=true;
							monst.remove();
							setTimeout(function(){
								monst.remove();
		...
		amounts.audio++;
		if(amounts.audio==levelObj.durabilityTools.audio){
	...
	document.getElementById('closeKletka').onclick=function(){
		...
		if(value){
			...
			if(typeof currentLevel=='number' && currentLevel!=10){
				...
				localStorage.setItem('ded', JSON.stringify({currentLevel,maxLevel, dop}))
				console.log('ded')
				location.reload()
...
function monitor(){
	...
	let t=setInterval(()=>{
		if(monitor1.style.display=='none')
			clearInterval(t)
		amounts.video++;
	}, 3000)
}
...
selectLevel.addEventListener('dragover', function(e) {    
	e.stopPropagation();    
	e.preventDefault();    
	e.dataTransfer.dropEffect = 'copy';
});
selectLevel.addEventListener('drop', function(e) {    
	e.stopPropagation();    
	e.preventDefault();    
	...
	filereader.onload=()=>{
		...
		levels.push(jsObject);
		currentLevel=jsObject.name
		addOption(jsObject.name)
		console.log(filereader.result);
	}
	
})
function closeBlendMonsters(){
	monsters.forEach((monst, k)=>{
		if(monst.type!=='normal'){
			monst.isAudio=true;
			monst.cloustrofobia=true
